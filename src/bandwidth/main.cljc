(ns bandwidth.main
  "Kotodama WASM entrypoint for the Bandwidth clean-room actor (L4) — Clojure port.
  ...
  facts (`bandwidth.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "bandwidth")
(def tier "L4")
...
(def entity-specs
  [{:entity "Message"   :plural "messages"   :id-prefix "bandwidt_mes"
    :fields [:owner :applicationId :time :segmentCount :direction :toNumber :fromNumber :text :tag]
    :required [:owner]
    :coerce {:segmentCount :int}     :refs {}}
   {:entity "Call"      :plural "calls"      :id-prefix "bandwidt_cal"
    :fields [:callId :accountId :applicationId :toNumber :fromNumber :state :direction :createdTime :startTime :endTime]
    :required [:toNumber :fromNumber]
    :coerce {}                       :refs {}}
   {:entity "Conference" :plural "conferences" :id-prefix "bandwidt_con"
    :fields [:name :status :redirectUrl :createdTime] :required [:name]
    :coerce {}                       :refs {}}
   {:entity "Recording" :plural "recordings" :id-prefix "bandwidt_rec"
    :fields [:recordingId :applicationId :accountId :name :status :fileFormat :startTime :endTime :duration]
    :required [:name :status]
    :coerce {:duration :int}         :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"bandwidth.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "bandwidth-compat" :tier tier :entities entities} 200])
...
(defn start! [] :bandwidth-compat/ready)
